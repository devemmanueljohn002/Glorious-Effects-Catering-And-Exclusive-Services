...
export default function ResourceManager({ title, description, singular, columns, fields, initialRows, statuses }: { title: string; description: string; singular: string; columns: Column[]; fields: Field[]; initialRows: Row[]; statuses?: string[] }) {
  ...
  return <div className="dashboard-content">
    ...
    <section className="management-stats"><div><span>Total {title.toLowerCase()}</span><strong>{rows.length}</strong></div><div><span>Active records</span><strong>{rows.filter((row) => !['cancelled','inactive','lost','unavailable'].includes(String(row.status).toLowerCase())).length}</strong></div><div><span>Needs attention</span><strong>{rows.filter((row) => ['pending','new','low stock','in review'].includes(String(row.status).toLowerCase())).length}</strong></div></section>
